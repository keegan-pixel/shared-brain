...
| `get_document` MCP tool | ✅ Complete | 2026-05-08 | Closes a real gap caught testing on mobile: `search` and `get_wiki_pages` return previews; nothing surfaced the F2-extracted full text of `.docx/.pdf/.xlsx` files. New tool reads `extractedText` (or `content` for prose) by id or title match, with a 100K-char default cap. Profile.md updated so Claude reaches for it after `search`. |
