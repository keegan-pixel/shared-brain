...
| 8 v1 — OAuth on `/api/mcp` | ✅ Complete | 2026-05-08 | OAuth 2.1 Authorization Code + PKCE (S256). Discovery doc at `/.well-known/oauth-authorization-server`; consent page at `/authorize` (Clerk-protected); token exchange at `/api/oauth/token`. MCP handler accepts either `MCP_API_KEY` or `sb_at_…` access tokens; unauthenticated reqs return `WWW-Authenticate` pointing at the discovery doc. `npm run create-oauth-client` CLI for manual client registration. ADR-034. |
| Desktop migrated off `mcp-remote` | ✅ Complete | 2026-05-08 | Removed the stdio bridge entry from `claude_desktop_config.json`. Desktop inherits the OAuth Custom Connector via account state — same connector as claude.ai web. Closes the disconnect-prone bridge story (ADR-035). |
| `get_document` + `get_document_url` MCP tools | ✅ Complete | 2026-05-08 | Closes the mobile find/view/save/forward gap: `search` and `get_wiki_pages` return previews only; nothing surfaced the F2-extracted full text or tappable URLs. `get_document` returns full extracted text + `view_url`/`download_url`/`preview_url` (Clerk-auth'd). `get_document_url` is a low-token variant that returns ONLY the URL set — for "send me the brand doc"-style requests where reading wastes tokens. Profile.md gained primitive-reference (no recipes per ADR-033). Primitives-only — no `email_document` workflow tool. |
| Search/list URL enrichment | ✅ Complete | 2026-05-08 | `search` and `get_wiki_pages` now return `view_url` + `download_url` + `preview_url` + `kind: binary\|prose` on every result. "Show me X" / "send me X" answerable in ONE tool call by surfacing the URL — no follow-up `get_document` needed unless reading. Tightened tool descriptions to beat Composio's `COMPOSIO_SEARCH_TOOLS` collision and explicitly mark URLs as TAPPABLE-NOT-FETCHABLE (Claude was wasting calls trying to fetch Clerk-auth'd URLs server-side). |
| Binary file storage gap fix | ✅ Complete | 2026-05-08 | Caught from mobile testing: PNGs and DOCXs added to vault since 2026-04-30 had wiki entries with `blob_url IS NULL` — search said `kind: prose`, view_url went to `/wiki/<id>` (metadata page), tap on mobile was useless. Two combined root causes: (1) `agent/src/index.ts` watch handler filtered to `.md` only, silently dropping binaries during watch; (2) `scripts/install-daemon.ts` plist didn't forward `BLOB_READ_WRITE_TOKEN` so upload silently fell through `isBlobConfigured() === false`. Both fixed; `npm run backfill:blob-urls` recovered 36 files (24 PNGs / 14 DOCX / 1 PDF / 1 SVG). ADR-036. |
| Mobile sidebar collapse | ✅ Complete | 2026-05-08 | Sidebar took ~50% of phone viewport, made `/wiki/<id>` and `/api/files/<id>/preview` unreadable on mobile after tapping a URL from a Custom Connector tool response. New `mobile-sidebar.tsx` (drawer/sheet), `mobile-nav-context.tsx` (open/close state), `sidebar-data.ts` (shared nav data); topbar shows hamburger below 768px. |
| 8 v2 MVP — Build A: Per-user org isolation + rename | ✅ Complete | 2026-05-11 | `ensureUserOrg()` now reads Clerk user info and defaults new orgs to `{first} {last}'s Brain`. Org rename UI at `/settings/org` (name + Obsidian vault name). Slug derived with collision-suffix logic; slug stays stable on rename so URLs don't break. `organizations.vault_name` column added (migration 0008). Backfilled "ViaOps" for Keegan's existing org. ADR-037. |
| 8 v2 MVP — Build B: Per-org LLM API keys | ✅ Complete | 2026-05-11 | `org_llm_config` table (provider, api_key, default_model, use_for[], monthly_token_cap). `resolveOrgLlmKey()` resolver walks provider preference order, matches on use_for ('all' or specific), falls back to env vars for legacy. `validateLlmKey()` hits provider's models endpoint with 5s timeout — bad keys rejected at save time. `/settings/llm-keys` UI: per-provider cards (Anthropic / OpenAI / Gemini) with "Get a key →" links + validate-and-save flow. Embeddings.ts + chat route + filing engine all swapped to per-org resolution. ADR-037. |
| 8 v2 MVP — Build C: Per-org Composio key | ✅ Complete | 2026-05-11 | `org_composio_config` table (one row per org with api_key + optional mcp_url override). `resolveOrgComposioKey()` + `validateComposioKey()` (hits Composio's MCP `initialize` for a quick auth check). `/settings/connections` UI: paste + validate flow with step-by-step Composio signup guidance. `composio-tools.ts` caches client per-orgId. Chat route + sync watchers updated to pass orgId. Per-connection routing flagged as v2.1 follow-up. ADR-037. |
| 8 v2 MVP — Build D: Daemon plist namespacing + per-org sync keys | ✅ Complete | 2026-05-11 | `install-daemon.ts` gains `--user-tag <slug>` flag; plist label becomes `com.shared-brain.sync.<slug>.plist` for new installs. When `--user-tag` is omitted, falls back to legacy label (`com.viaops.shared-brain.sync`) so Keegan's existing daemon keeps working. `organizations.mcp_api_key` column added (per-org sync key, auto-generated on org create, format `sb_sync_<base64url>`). `requireSyncAuth` resolves the org from the presented key (Path 1: per-org match; Path 2: legacy env-var match → first org). Sync routes use the returned orgId instead of `resolveSyncOrg()`. Backfilled Keegan's org with a fresh sync key. ADR-037. |
| 8 v2 MVP — Build E: Onboarding checklist + daemon/Claude pages | ✅ Complete | 2026-05-11 | Dashboard (`/`) shows a 5-step checklist for new users: org named, LLM keys, Composio, daemon connected, Claude connected. State derived from existing tables (no new "onboarding_state" table for v2.0 MVP). `/settings/daemon`: reveal-on-tap sync key + vault-path input → generates a one-line install command (git clone + npm install + install-daemon). `/settings/claude`: MCP server URL + step-by-step Custom Connector setup. |
| 8 v2 MVP — Build F: Claude Project Instructions generator | ✅ Complete | 2026-05-11 | Template at `lib/claude-project-template.ts` renders personalized markdown per org: connection statement, primitives reference, first-run discovery interview (10-15 conversational questions Claude runs to set up spaces/projects via primitives — per ADR-033, no recipes for general use, just discovery structure), live Profile.md content, first-conversation suggestions, behavior reminders. Download CTA on `/settings/claude` returns the .md as an attachment. |
| 8 v2 MVP — Jake's install (first non-Keegan user) | ✅ Complete | 2026-05-12 | **First real multi-tenant test.** 16 bugs hit live during the install + 3 caught proactively; 19 fixes shipped (commits `07bfdf7` → `cc28bb8` + tonight's MF batch). Most critical: AsyncLocalStorage for per-user MCP context (`595c1de`) — Jake's Claude was seeing Keegan's data before the fix. Plus DCR endpoint (`5af980b`), daemon include path generalization (`398ccb1` + `e465b21` + `ffb4733`), Composio MANAGE_CONNECTIONS direct-call helper + parser (`2ee38f6` → `b3501a5` → `cc28bb8`). Outcome: Jake's brain live, his Claude sees only his data, daemon syncing his files, lead-agent patch installed. ADR-038 codifies the discipline rules. Full retro: `docs/post-mortem-2026-05-12.md`. |
| Multi-tenancy hardening — Must-fixes after Jake's install | ✅ Complete | 2026-05-12 | **MF-1:** `deriveSpaceIdFromPath` (`/api/sync/wiki`) generalized — looks up real space names in user's org instead of Keegan-hardcoded `Clients/SimHouse.io/Coaching`. **MF-2:** Slugify (`src/lib/org.ts` + `scripts/install-daemon.ts`) strips `'s` possessive before slug split — no more `jake-leskovar-s-brain`. **MF-3:** `/settings/org` placeholder generic ("e.g. MyVault" not "e.g. ViaOps"). **MF-4:** MANAGE_CONNECTIONS toolkit list trimmed to 25 stable toolkits (dropped granola, onedrive, evernote, twitter, tiktok that errored during Jake's debug) + amber UI warning explains the 10-min auth-link pollution side effect. **MF-5:** Obsidian deep-link `vault=ViaOps` was hardcoded in `agent/src/sync.ts` — caught by ADR-038 audit grep. Now reads `OBSIDIAN_VAULT_NAME` env; install-daemon accepts `--vault-name`; `/settings/daemon` install command UI passes the org's `vault_name` automatically. Skips the link entirely when unset. Pre-deploy audit per ADR-038 Rule 1 passed. |
| 8 v2 v2.1+ — Multi-tenancy proper (memberships, invites, visibility) | 🅿️ Parked | — | `org_memberships` table, invite flow, role-based permissions, multi-org doc membership (the Venn diagram). Required when a second org or first team-org user signs up. Specced in [[Phase 8 v2 — Multi-User + Onboarding Spec]] §17 (Epics 1, 7, 8). |
