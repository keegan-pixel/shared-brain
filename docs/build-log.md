...
| F4 v2 — Config UI + cron auto-sync | ⏳ Queued | — | `sync_configs` table + `/settings/sync` page; per-Composio-connection toggle (off/manual/auto), schedule + source filter; cron walks active configs, polls Composio for new items, calls F4 v1's `file_document`. |
| F4 v3 — Active-learning reconciliation | ⏳ Queued | — | Detect when user moves an Inbox-routed file to a different folder; record the transition as a `filing_rules` rule (e.g. "emails from `mark@xpflow.com` → `Clients/XP Flow/Meetings/`"). F4 v1's classifier consults rules first on subsequent files. Compounds without re-prompting. |
